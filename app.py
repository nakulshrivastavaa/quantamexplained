...
def set_background():
    ...
    .st-emotion-cache-1g6gooi { /* This class might change with Streamlit updates */
        color: #a0a0a0;
    }
    </style>
    """
...
def display_path_choice():
    """Displays the radio buttons for path selection."""
...
def display_peace_path():
    ...
    st.info(f"Just {mass_grams}g of mass, if fully converted to energy, could power an average US household for over {int(energy_joules / (3.6e6 * 8760))} years!")
...
def display_destruction_path():
    ...
    # More engaging impact data chart
    st.subheader("📊 Impact Data: The Human Cost")
    data = pd.DataFrame({
        "City": ["Hiroshima", "Nagasaki"],
        "Initial Fatalities": [140000, 74000],
    })
    fig = go.Figure(data=[go.Bar(
        x=data['City'], 
        y=data['Initial Fatalities'],
        marker_color=['#ff6b6b', '#ff8e8e'],
        text=data['Initial Fatalities'],
        textposition='auto',
    )])
    fig.update_layout(
        title_text='Estimated Immediate Deaths',
        xaxis_title="City",
...
def display_quantum_concepts():
    """The section for core quantum concepts and animations."""
    ...
    st.subheader("🔬 Visualizing Atomic Structure (Smoother Animation)")
    t = st.session_state.frame * 0.02
    
    nucleus = go.Scatter(x=[0], y=[0], mode='markers', marker=dict(size=40, color='#ff6b6b', symbol='circle'), name='Nucleus')
    ...
    st.markdown("A quantum object can behave as a wave or a particle, but not both at the same time. The act of observation determines its nature.")
    ...
    with col2:
        ...
        if lottie_particle:
            st_lottie(lottie_particle, height=200, key="particle")
            
    # --- Quantum Tunneling ---
    st.subheader("👻 Quantum Tunneling")
    st.markdown("Particles can pass through energy barriers they classically shouldn't be able to overcome, as if they've 'tunneled' through.")
    ...
    fig_tunnel.update_layout(
        yaxis=dict(visible=False, range=[-2, barrier_height + 2]),
...
# --- Footer ---
st.markdown("---", unsafe_allow_html=True)
st.caption("Created by Nakul | Enhanced in Python by Gemini | Inspired by the profound ethics of scientific discovery.")
